...
public class CoverLoader {
    ...
    public void setRoundLength(int roundLength){
        if(this.roundLength != roundLength){
            ...
            cacheMap.get(Type.ROUND).evictAll();//清除缓存
        }
    }

    public Bitmap loadThumb(Music music){return loadCover(music, Type.THUMB);}

    public Bitmap loadRound(Music music){return  loadCover(music,Type.ROUND);}

    public Bitmap loadBlur(Music music){return  loadCover(music,Type.BLUR);}
    ...
    private Bitmap getDefaultCover(Type type){
            switch (type){
                case ROUND:
                    ...
                    return bitmap;//将封面裁剪
                case BLUR:
                    return BitmapFactory.decodeResource(context.getResources(),R.drawable.play_page_bg_default);
                    default:
    ...
    private Bitmap loadCoverByType(Music music,Type type){
        ...
        switch (type){
            case ROUND:
                bitmap = ImageUtils.resizeImage(bitmap,roundLength,roundLength);
                return  ImageUtils.createCircleImage(bitmap);//裁剪成圆形
            case BLUR:
